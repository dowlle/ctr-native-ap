...
static inline int AP_BoxEdge_RectFromLayout(const unsigned char uv[8], unsigned tpage, unsigned clut, AP_BoxEdgeRect *out)
{
...
static inline int AP_BoxEdge_IsWoodRect(const AP_BoxEdgeRect *r)
{
...
static inline const unsigned char *AP_BoxEdge_VramAt(const unsigned char *vrm, int vrmSize, int x, int y, int *strideOut)
{
	int pos = 4;
...
static inline unsigned AP_BoxEdge_Half(const unsigned char *p)
{
...
static inline int AP_BoxEdge_Decode4bpp(const unsigned char *vrm, int vrmSize, const AP_BoxEdgeRect *r, unsigned char *dst,
                                 int dstStride)
{
...
static inline void AP_BoxEdge_Recolour(unsigned char *rgba, int w, int h, int stride)
{
...
// The same recolour onto a ramp derived from one target colour instead of the
// fixed pink: shadow 40%, dark grain 66%, wood 84% of the colour, highlights
// the colour lifted 15% toward white. Used for the item classification colours
// (ap_box_colour_logic.h). Same grey and transparency rules as the pink.
...
// The AP box's face is the retail Wumpa crate's face (slats, no picture), read
// from the player's disc like the wood border, recoloured the same way, with
// JurnthReinal's six Archipelago circles on top. If that read fails the face
// stays JurnthReinal's own art.

// The retail Wumpa crate's face is its only 64x64 4bpp rect.
static inline int AP_BoxEdge_IsFaceRect(const AP_BoxEdgeRect *r)
{
...
static inline void AP_BoxEdge_ComposeLogo(unsigned char *face, int faceStride, const unsigned char *crate, int crateStride,
