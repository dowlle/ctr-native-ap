...
void VehPhysForce_OnGravity(struct Driver *driver, Vec3 *velocity)
{
	...
	if (((rotatedGravity.z < 0) && (driver->unk_offset3B2 > 0)) || ((rotatedGravity.z > 0) && (driver->unk_offset3B2 < 0)))
	{
		rotatedGravity.z = 0;
	}

	if ((driver->actionsFlagSet & ACTION_PREVENT_ACCEL) || ((driver->baseSpeed > 0) && (driver->speedApprox < 0)) ||
