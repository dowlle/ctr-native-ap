#include <common.h>

CTR_STATIC_ASSERT(sizeof(struct OverlayRDATA_229) == 0x420);
CTR_STATIC_ASSERT(sizeof(struct OverlayRDATA_229_BucketSetupRecord) == 0x64);
CTR_STATIC_ASSERT(sizeof(((struct OverlayRDATA_229 *)0)->scratchInitTable) == 0x60);
CTR_STATIC_ASSERT(sizeof(((struct OverlayRDATA_229 *)0)->clipRecordJumpTable) == 0x60);

struct OverlayRDATA_229 R229 = {
    .bucketSetupAddresses =
        {
            OVR229_RDATA_BUCKET_SETUP_ADDRESS(5),
            OVR229_RDATA_BUCKET_SETUP_ADDRESS(4),
            OVR229_RDATA_BUCKET_SETUP_ADDRESS(3),
            OVR229_RDATA_BUCKET_SETUP_ADDRESS(2),
            OVR229_RDATA_BUCKET_SETUP_ADDRESS(1),
            OVR229_RDATA_BUCKET_SETUP_ADDRESS(0),
            OVR229_RDATA_BUCKET_SETUP_ADDRESS(7),
            OVR229_RDATA_BUCKET_SETUP_ADDRESS(6),
        },

    .bucketHandlerAddresses =
        {
            OVR229_RETAIL_LABEL_SPLIT_GROUND_RENDERED_B_HANDLER,
            OVR229_RETAIL_LABEL_SPLIT_GROUND_LIST_B_HANDLER,
            OVR229_RETAIL_LABEL_SPLIT_GROUND_RENDERED_A_HANDLER,
            OVR229_RETAIL_LABEL_SPLIT_GROUND_LIST_A_HANDLER,
            OVR229_RETAIL_LABEL_WATER_RENDERED_HANDLER,
            OVR229_RETAIL_LABEL_WATER_BSP_LIST_HANDLER,
            OVR229_RETAIL_LABEL_QUAD_4X4_RENDERED_HANDLER,
            OVR229_RETAIL_LABEL_WIDE_DYNAMIC_HANDLER,
        },

    .scratchInitTable =
        {
            0x18100800, 0x80900818, 0x00081018, 0x98881000, 0x98081000, 0x00881018, 0x80100818, 0x18900800, 0x10081898, 0x88881810, 0x18180810, 0x90980888,
            0x90180888, 0x18980810, 0x88081810, 0x10881898, 0x08181090, 0x98981008, 0x10101808, 0x88901898, 0x88101898, 0x10901808, 0x98181008, 0x08981090,
        },

    .bucketSetups =
        {
            {
                {{0xe, OVR229_RDATA_BUCKET_COPY0_ADDRESS(0), 0x14c}, {0x2, OVR229_RDATA_BUCKET_COPY1_ADDRESS(0), 0x188}},
                0x0,
                {
                    0x800a1680,
                    0x800a16c0,
                    0x800a1740,
                    0x800a16e0,
                    0x800a1768,
                    0x800a17e0,
                    0x800a17e0,
                    0x800a1700,
                    0x800a17e0,
                    0x800a1790,
                    0x800a17e0,
                    0x800a17b8,
                    0x800a17e0,
                    0x800a17e0,
                    0x800a17e0,
                },
                {
                    0x800a1b14,
                    0x800a1b0c,
                    0x800a1b88,
                },
            },
            {
                {{0xe, OVR229_RDATA_BUCKET_COPY0_ADDRESS(1), 0x14c}, {0x2, OVR229_RDATA_BUCKET_COPY1_ADDRESS(1), 0x188}},
                0x0,
                {
                    0x800a2178,
                    0x800a21b8,
                    0x800a2238,
                    0x800a21d8,
                    0x800a2260,
                    0x800a22d8,
                    0x800a22d8,
                    0x800a21f8,
                    0x800a22d8,
                    0x800a2288,
                    0x800a22d8,
                    0x800a22b0,
                    0x800a22d8,
                    0x800a22d8,
                    0x800a22d8,
                },
                {
                    0x800a2670,
                    0x800a2668,
                    0x800a2714,
                },
            },
            {
                {{0xe, OVR229_RDATA_BUCKET_COPY0_ADDRESS(2), 0x14c}, {0x2, OVR229_RDATA_BUCKET_COPY1_ADDRESS(2), 0x188}},
                0x0,
                {
                    0x800a2f68,
                    0x800a2fa8,
                    0x800a3028,
                    0x800a2fc8,
                    0x800a3050,
                    0x800a30c8,
                    0x800a30c8,
                    0x800a2fe8,
                    0x800a30c8,
                    0x800a3078,
                    0x800a30c8,
                    0x800a30a0,
                    0x800a30c8,
                    0x800a30c8,
                    0x800a30c8,
                },
                {
                    0x800a3724,
                    0x800a371c,
                    0x800a37a8,
                },
            },
            {
                {{0xe, OVR229_RDATA_BUCKET_COPY0_ADDRESS(3), 0x14c}, {0x2, OVR229_RDATA_BUCKET_COPY1_ADDRESS(3), 0x188}},
                0x0,
                {
                    0x800a3c70,
                    0x800a3cb0,
                    0x800a3d30,
                    0x800a3cd0,
                    0x800a3d58,
                    0x800a3dd0,
                    0x800a3dd0,
                    0x800a3cf0,
                    0x800a3dd0,
                    0x800a3d80,
                    0x800a3dd0,
                    0x800a3da8,
                    0x800a3dd0,
                    0x800a3dd0,
                    0x800a3dd0,
                },
                {
                    0x800a4490,
                    0x800a4488,
                    0x800a4544,
                },
            },
            {
                {{0xe, OVR229_RDATA_BUCKET_COPY0_ADDRESS(4), 0x14c}, {0x2, OVR229_RDATA_BUCKET_COPY1_ADDRESS(4), 0x188}},
                0x0,
                {
                    0x800a4da8,
                    0x800a4de8,
                    0x800a4e68,
                    0x800a4e08,
                    0x800a4e90,
                    0x800a4f08,
                    0x800a4f08,
                    0x800a4e28,
                    0x800a4f08,
                    0x800a4eb8,
                    0x800a4f08,
                    0x800a4ee0,
                    0x800a4f08,
                    0x800a4f08,
                    0x800a4f08,
                },
                {
                    0x800a5564,
                    0x800a555c,
                    0x800a55e8,
                },
            },
            {
                {{0xe, OVR229_RDATA_BUCKET_COPY0_ADDRESS(5), 0x14c}, {0x2, OVR229_RDATA_BUCKET_COPY1_ADDRESS(5), 0x188}},
                0x0,
                {
                    0x800a5ab0,
                    0x800a5af0,
                    0x800a5b70,
                    0x800a5b10,
                    0x800a5b98,
                    0x800a5c10,
                    0x800a5c10,
                    0x800a5b30,
                    0x800a5c10,
                    0x800a5bc0,
                    0x800a5c10,
                    0x800a5be8,
                    0x800a5c10,
                    0x800a5c10,
                    0x800a5c10,
                },
                {
                    0x800a62d0,
                    0x800a62c8,
                    0x800a6384,
                },
            },
            {
                {{0xe, OVR229_RDATA_BUCKET_COPY0_ADDRESS(6), 0x14c}, {0x2, OVR229_RDATA_BUCKET_COPY1_ADDRESS(6), 0x188}},
                0x0,
                {
                    0x800a6a0c,
                    0x800a6a4c,
                    0x800a6acc,
                    0x800a6a6c,
                    0x800a6af4,
                    0x800a6b6c,
                    0x800a6b6c,
                    0x800a6a8c,
                    0x800a6b6c,
                    0x800a6b1c,
                    0x800a6b6c,
                    0x800a6b44,
                    0x800a6b6c,
                    0x800a6b6c,
                    0x800a6b6c,
                },
                {
                    0x800a7168,
                    0x800a7160,
                    0x800a71ec,
                },
            },
            {
                {{0xe, OVR229_RDATA_BUCKET_COPY0_ADDRESS(7), 0x14c}, {0x2, OVR229_RDATA_BUCKET_COPY1_ADDRESS(7), 0x188}},
                0x0,
                {
                    0x800a76b4,
                    0x800a76f4,
                    0x800a7774,
                    0x800a7714,
                    0x800a779c,
                    0x800a7814,
                    0x800a7814,
                    0x800a7734,
                    0x800a7814,
                    0x800a77c4,
                    0x800a7814,
                    0x800a77ec,
                    0x800a7814,
                    0x800a7814,
                    0x800a7814,
                },
                {
                    0x800a7ed4,
                    0x800a7ecc,
                    0x800a7f88,
                },
            },
        },

    .clipRecordJumpTable =
        {
            0x800a844c, 0x800a8464, 0x800a849c, 0x800a84d4, 0x800a8508, 0x800a8540, 0x800a8574, 0x800a85a8, 0x800a8a50, 0x800a8a6c, 0x800a8ab4, 0x800a8b0c,
            0x800a8b60, 0x800a8bb8, 0x800a8c0c, 0x800a8c6c, 0x800a8ca0, 0x800a8ce8, 0x800a8d50, 0x800a8da4, 0x800a8de8, 0x800a8e3c, 0x800a8e80, 0x800a8eb4,
        },
};
