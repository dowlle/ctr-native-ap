...
carries a `.gnu_debuglink` pointing at the sidecar, so `gdb` on the Deck or the
build host auto-loads the symbols as long as the `.debug` file stays next to
the binary. Keep the pair together for any debugging work. Anyone who copies the
