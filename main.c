...
int main(int argc, char *argv[])
{
	...
	if (!startupArgs.validateDiscOnly)
	{
		...
		case NATIVE_LINK_ROUTE_HAND_OFF:
		{
			...
			NativeLinkPendingStatus published;

			memset(&record, 0, sizeof record);
			record.request = launchRequest;
			record.token = NativeLinkHost_NewToken();
			record.createdUnixMs = NativeLinkHost_NowUnixMs();
			NativeLinkHost_Ops(linkHost, &linkOps);
			published = NativeLinkPending_Publish(&linkOps, &record, record.createdUnixMs);
			printf("[CTR-AP] %s\n", NativeLinkPending_StatusText(published));
			fflush(stdout);
			return NativeConsole_Return((published == NATIVE_LINK_PENDING_PUBLISHED ||
			                             published == NATIVE_LINK_PENDING_REPLACED ||
			                             published == NATIVE_LINK_PENDING_COALESCED) ? 0 : 1);
		}
